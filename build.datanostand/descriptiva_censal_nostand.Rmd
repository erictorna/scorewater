...
setDT(conditions_gral_censal)
conditions_gral_censal[is.na(conditions_gral_censal)] <- 0
...
```{r}
autofit(flextable(conditions_gral_censal[,.(n=sum(casos_prevalents ,na.rm = T)),.(provincia)]))
```
...
n_province = conditions_gral_censal[,.(n=sum(casos_prevalents ,na.rm = T)),.(province = provincia)][]
...
```{r}
autofit(flextable(conditions_gral_censal[,.(N=sum(casos_prevalents ,na.rm = T)),.(condicio)]))
```
...
```{r, fig.width = 7, fig.height=4.5, fig.align='center'}
n_conditions = conditions_gral_censal[,.(equal=sum(casos_prevalents ,na.rm = T)),.(condicio = condicio)][]
...
conditions_gral_censal[, condicio := fstat(condicio)]
autofit(flextable(dcast(conditions_gral_censal[, sum(casos_prevalents ,na.rm = T), keyby = .(condicio, provincia)], condicio ~ provincia)))
```
...
```{r, fig.align='center'}
data = conditions_gral_censal[, (sum(casos_prevalents ,na.rm = T)/(sum(poblacio_prevalent ,na.rm = T))), keyby = .(condicio, provincia)]
...
conditions_gral_censal[, condicio := fstat(condicio)]
# autofit(flextable(dcast(conditions_gral_censal[, sum(casos_prevalents ,na.rm = T), keyby = .(condicio, scensal)], condicio ~ scensal)))
```
...
```{r, results='asis'}
data = conditions_gral_censal[, prev, keyby = .(condicio, scensal, provincia, casos_prevalents, poblacio_prevalent, perc_censat)]
data1 <- data %>% 
  group_by(condicio, scensal, provincia) %>%
...
for(DISEASE in EV){
    ...
    scensaldata <- df[with(df,order(-prev)),]
    ...
    print(ggplot() + 
            geom_bar(data = scensaldata, aes(x=scensal, y=prev, fill = provincia), stat = 'identity') +
            theme_minimal() +
            ggtitle(DISEASE) +
...
* Only the municipality with the highest prevalence value in each quantile is used
```{r, results='asis'}
data = conditions_gral_censal[, prev, keyby = .(condicio, scensal, provincia, poblacio_prevalent, perc_censat)]
data <- data %>% 
  group_by(condicio, scensal, provincia) %>%
  summarize_if(is.numeric, mean, na.rm = T)
numberlist = c(20:1)
for(DISEASE in EV){
  ...
  for (number in numberlist) {
    df$quartile <- ntile(df$prev, 20)  
    quartils <- df[df$quartile == number, ] 
    table = rbind(table, quartils[which.max(quartils$prev),])
  }
  table$scensal <- factor(table$scensal, levels = table$scensal)
  print(ggplot() + 
          geom_bar(data = table, aes(x=scensal, y=prev, fill = provincia), stat = 'identity') +
          theme_minimal() +
          ggtitle(DISEASE) +
          labs(x = 'Censal code', y = ' prevalence') +
          theme(axis.text.x = element_text(angle=45, hjust = 1)))
...
```{r, results='asis'}
data = conditions_gral_censal[, prev, keyby = .(condicio, scensal, provincia, poblacio_prevalent)]
data1 <- data %>% 
  group_by(condicio, scensal, provincia) %>%
  summarize_if(is.numeric, mean, na.rm = T)
data1$poblacio_prevalent <- NULL
...
data1$poblacio_prevalent <- data2$poblacio_prevalent
data <- data1
a = aggregate(data$poblacio_prevalent, by = list(data$scensal), max)
setDT(a)
a$scensal <- a$Group.1
a$Group.1 <- NULL
data$poblacio_prevalent<-NULL
...
res
```
### Correlation between Colon cancer and Thyroid function by municipi
...
```{r}
data = conditions_gral_censal %>% dplyr::select(scensal, condicio, prev)

data = dcast(data, scensal ~ condicio, fill = 0)
z <- data[,-c(1,1)]
...
# Seleccionem només les variables més importants (>0.01) + Colon cancer, Obesity i Thyroides function en cas que no hi siguin
