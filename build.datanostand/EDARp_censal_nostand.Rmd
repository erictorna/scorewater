...
load('conditions_censal_nostand_EDAR.RData')
...
```{r, results='asis'}
data = conditions_gral_censal[, prev, keyby = .(condicio, municipi, scensal, poblacio_prevalent, perc_censat)]
data <- data %>% 
...
for(DISEASE in EV){
  ...
  print(ggplot() + 
        geom_bar(data = table, aes(x=scensal, y=prev, fill = quantile), stat = 'identity') +
        theme_minimal() +
        ggtitle(DISEASE) +
