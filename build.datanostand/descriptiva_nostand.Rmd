...
load('conditions_municipi_nostand.RData')
setDT(conditions_gral_municipal)
conditions_gral_municipal[is.na(conditions_gral_municipal)] <- 0
...
```{r}
autofit(flextable(conditions_gral_municipal[,.(n=sum(casos_prevalents ,na.rm = T)),.(provincia)]))
```
...
n_province = conditions_gral_municipal[,.(n=sum(casos_prevalents ,na.rm = T)),.(province = provincia)][]
...
```{r}
autofit(flextable(conditions_gral_municipal[,.(N=sum(casos_prevalents ,na.rm = T)),.(condicio)]))
```
...
```{r, fig.width = 7, fig.height=4.5, fig.align='center'}
n_conditions = conditions_gral_municipal[,.(equal=sum(casos_prevalents ,na.rm = T)),.(condicio = condicio)][]
...
conditions_gral_municipal[, condicio := fstat(condicio)]
autofit(flextable(dcast(conditions_gral_municipal[, sum(casos_prevalents ,na.rm = T), keyby = .(condicio, provincia)], condicio ~ provincia)))
```
...
```{r, fig.align='center'}
data = conditions_gral_municipal[, (sum(casos_prevalents ,na.rm = T)/(sum(poblacio_prevalent ,na.rm = T))), keyby = .(condicio, provincia)]
...
```{r, results='asis'}
data = conditions_gral_municipal[, prev, keyby = .(condicio, municipi, provincia, perc_censat)]
data1 <- data %>% 
...
for(DISEASE in EV){
    ...
    print(ggplot() + 
            geom_bar(data = scensaldata, aes(x=municipi, y=prev, fill = provincia), stat = 'identity') +
            theme_minimal() +
            ggtitle(DISEASE) +
...
```{r, results='asis'}
data = conditions_gral_municipal[, prev, keyby = .(condicio, municipi, provincia, poblacio_prevalent, perc_censat)]
data <- data %>% 
  group_by(condicio, municipi, provincia) %>%
  summarize_if(is.numeric, mean, na.rm = T)
numberlist = c(20:1)
for(DISEASE in EV){
  ...
  print(ggplot() + 
          geom_bar(data = table, aes(x=municipi, y=prev, fill = provincia), stat = 'identity') +
          theme_minimal() +
          ggtitle(DISEASE) +
          labs(x = 'Municipi', y = ' prevalence') +
          theme(axis.text.x = element_text(angle=45, hjust = 1)))
...
```{r, results='asis'}
data = conditions_gral_municipal[, prev, keyby = .(condicio, municipi, provincia, poblacio_prevalent)]
data1 <- data %>% 
  group_by(condicio, municipi, provincia) %>%
  summarize_if(is.numeric, mean, na.rm = T)
data1$poblacio_prevalent <- NULL
...
```{r}
data = conditions_gral_municipal %>% dplyr::select(municipi, condicio, prev)
