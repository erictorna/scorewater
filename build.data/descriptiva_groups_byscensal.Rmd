...
load('groups_standardized_byscensal.RData')
setDT(groups_gral)
...
```{r, include=FALSE}
# Total Number of every condition * scensal
a = groups_gral[, .N, keyby = .(subgrup)]
condiciolist = a[[1]]
fstat = function(x) factor(x, labels = condiciolist)
groups_gral[, subgrup := fstat(subgrup)]
# kable(dcast(groups_gral[, sum(casos_prevalents ,na.rm = T), keyby = .(subgrup, scensal)], subgrup ~ scensal))
```

## Tables
### 10 populations with higher prevalence value
```{r, results='asis'}
data = groups_gral[, stand., keyby = .(subgrup, scensal, provincia, casos_prevalents, poblacio_prevalent, perc_censat)]
data1 <- data %>% 
  group_by(subgrup, scensal, provincia) %>%
...
## Qantiles
### 20 quantiles 
* Displaying the censal code unit with highest prevalence value of every quantile
```{r, results='asis'}
data = groups_gral[, stand., keyby = .(subgrup, scensal, provincia, poblacio_prevalent, perc_censat)]
data <- data %>% 
  group_by(subgrup, scensal, provincia) %>%
  summarize_if(is.numeric, mean, na.rm = T)
numberlist = c(20:1)
...
```{r}
data = groups_gral %>% dplyr::select(scensal, subgrup, stand.)

data = dcast(data, scensal ~ subgrup, fill = 0)
z <- data[,-c(1,1)]
