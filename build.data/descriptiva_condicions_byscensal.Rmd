...
conditions_gral[, condicio := fstat(condicio)]
autofit(flextable(dcast(conditions_gral[, sum(casos_prevalents ,na.rm = T), keyby = .(condicio, scensal)], condicio ~ scensal)))
```
...
```{r, results='asis'}
data = conditions_gral[, stand., keyby = .(condicio, scensal, provincia, casos_prevalents, poblacio_prevalent, perc_censat)]
data1 <- data %>% 
...
for(DISEASE in EV){
    ...
    print(ggplot() + 
            geom_bar(data = scensaldata, aes(x=scensal, y=stand., fill = provincia), stat = 'identity') +
            theme_minimal() +
            ggtitle(DISEASE) +
...
```{r, results='asis'}
data = conditions_gral[, stand., keyby = .(condicio, scensal, provincia, poblacio_prevalent, perc_censat)]
data <- data %>% 
  group_by(condicio, scensal, provincia) %>%
  summarize_if(is.numeric, mean, na.rm = T)
numberlist = c(20:1)
for(DISEASE in EV){
  ...
  for (number in numberlist) {
    ...
    table = rbind(table, quartils[which.max(quartils$stand.),])
  }
  table$scensal <- factor(table$scensal, levels = table$scensal)
  print(ggplot() + 
          geom_bar(data = table, aes(x=scensal, y=stand., fill = provincia), stat = 'identity') +
          theme_minimal() +
          ggtitle(DISEASE) +
          labs(x = 'Censal code', y = ' prevalence') +
          theme(axis.text.x = element_text(angle=45, hjust = 1)))
...
```{r, results='asis'}
data = conditions_gral[, stand., keyby = .(condicio, scensal, provincia, poblacio_prevalent)]
data1 <- data %>% 
  group_by(condicio, scensal, provincia) %>%
  summarize_if(is.numeric, mean, na.rm = T)
data1$poblacio_prevalent <- NULL
...
```{r}
data = conditions_gral %>% dplyr::select(scensal, condicio, stand.)
