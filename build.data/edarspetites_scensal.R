...
conditions_gral <- conditions_gral %>% 
  group_by(scensal, provincia, municipi, condicio) %>%
  summarize_if(is.numeric, sum, na.rm = T)
...
conditions_gral_EDARSp$municipi <- NULL
conditions_gral_scensal = conditions_gral_scensal[!duplicated(conditions_gral_scensal), ]
conditions_gral_EDARSp <- merge(conditions_gral_EDARSp, conditions_gral_scensal, by = 'scensal')

# conditions_gral_EDARSp = conditions_gral_EDARSp[!duplicated(conditions_gral), ]



save(conditions_gral_EDARSp, file = '~idiap/projects/scorewater/build.data/conditions_standardized_EDARSp_scensal.RData')
